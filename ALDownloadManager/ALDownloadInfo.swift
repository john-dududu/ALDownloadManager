...
class ALDownloadInfo: NSObject {
    ...
    init(manager: Session?, destinationPath: String?, url: String? ) {
     ...
     */
    func createDestination(destinationPath: String?) -> Alamofire.DownloadRequest.Destination {
        let destination: Alamofire.DownloadRequest.Destination = { _, response in
